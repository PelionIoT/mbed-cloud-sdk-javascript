...
import { presubscriptionsEqual, } from "../../../legacy/connect/types";
...
export class ResourceValuesObserver extends Observer {
    ...
    compareData(data) {
        return this.localPresubscriptions.some(sub => {
            return (matchWithWildcard(sub.deviceId, data.deviceId) &&
    ...
    syncPresubscriptions() {
        if (this.connect) {
            this.connect
                ...
                .then(serverPresubscriptions => this.connect.updatePresubscriptions(this.unionOfPresubscriptions(serverPresubscriptions, this.localPresubscriptions)));
    ...
    unionOfPresubscriptions(server, local) {
