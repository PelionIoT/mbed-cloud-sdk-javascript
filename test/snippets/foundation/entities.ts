import { AccountRepository, ApiKeyRepository } from "../../../src/foundation";
...
describe("entities", () => {
    ...
    test("customConfig", () => {
        try {
            ...
            const allUsers = [];
            [ "ak_1", "ak_2" ].forEach(async k => allUsers.concat(await new Sdk(new Config({ apiKey: k })).foundation().userRepository().list().all()));
            // end of example
        } catch (e) {
            throw e;
        }
    ...
    test("realyCustomConfig", () => {
        try {
            ...
            const allUsers = new Sdk(config).foundation().userRepository().list().all();
            // end of example
