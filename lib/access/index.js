    ...
    */
    ...
    AccessApi.prototype.getAccount = function (callback) {
        ...
        return pg(function (done) {
            _this._endpoints.developer.getMyAccountInfo(null, function (error, data) {
                if (error)
                    return done(error);
    ...
    AccessApi.prototype.listUsers = function (options, callback) {
        ...
        return pg(function (done) {
            _this._endpoints.admin.getAllUsers(limit, after, order, data_1.encodeInclude(include), filter, function (error, data) {
                if (error)
                    return done(error);
    ...
    AccessApi.prototype.listCertificates = function (options, callback) {
        ...
        var limit = options.limit, after = options.after, order = options.order, include = options.include, filter = options.filter;
        return pg(function (done) {
            _this._endpoints.admin.getAllCertificates(limit, after, order, data_1.encodeInclude(include), filter, function (error, data) {
                if (error)
                    return done(error);
