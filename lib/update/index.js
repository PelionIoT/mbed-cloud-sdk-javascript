...
var UpdateApi = (function () {
    ...
    */
    function UpdateApi(options) {
        UpdateApi._endpoints = new endpoints_1.Endpoints(options);
    }
    UpdateApi.prototype.listFirmwareImages = function (options, callback) {
        ...
        return pg(function (done) {
            UpdateApi._endpoints.firmware.firmwareImageList(limit, order, after, data_1.encodeInclude(include), function (error, data) {
                if (error)
                    return done(error);
                var list = data.data.map(function (log) {
                    return firmwareImage_1.FirmwareImage.map(log);
                });
    ...
    UpdateApi.prototype.listFirmwareManifests = function (options, callback) {
        ...
        var limit = options.limit, order = options.order, after = options.after, include = options.include;
        return pg(function (done) {
            UpdateApi._endpoints.firmware.firmwareManifestList(limit, order, after, data_1.encodeInclude(include), function (error, data) {
                if (error)
                    return done(error);
                var list = data.data.map(function (log) {
                    return firmwareManifest_1.FirmwareManifest.map(log);
                });
                done(null, data_1.mapListResponse(data, list));
            });
        }, callback);
