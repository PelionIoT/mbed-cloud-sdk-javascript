...
suite("testBillingMonth", function () {
    test("singleDigitMonth", function () {
        var date = new Date(2018, 4);
        var string = functions_1.dateToBillingMonth(date);
        assert.strictEqual("2018-05", string);
    });
    test("doubleDigitMonth", function () {
        var date = new Date(2018, 11);
        var string = functions_1.dateToBillingMonth(date);
